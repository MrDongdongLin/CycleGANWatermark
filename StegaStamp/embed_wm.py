...
parser.add_argument(
    "--identical_watermarks", type=bool, default=True,
    help="If this option is provided use identical watermarks. Otherwise sample arbitrary watermarks."
)
parser.add_argument(
...
def embed_watermarks():
    ...
    for i, (images, _) in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        if args.check:
            ...
            bitwise_accuracy += (
                        detected_watermarks[: images.size(0)].detach() == watermarks[: images.size(0)]).float().mean(
                dim=1).sum().item()

            wm_img = watermarked_images.detach().cpu()
            for j in range(wm_img.size(0)):
                save_image(wm_img[j, :, :, :], os.path.join(args.output_dir, "%05d%05d.png" % (i, j)), padding=0)
            # save_image(wm_img, os.path.join(args.output_dir, "%05d.png" % i), padding=0)

            psnr_bs = PSNR()(images.cpu().detach(), watermarked_images.cpu().detach())
            ...
            # test it if needed
            # if i == 2:
            #     save_image(images[:args.nrow * args.nrow], os.path.join(args.output_dir, "test_samples_clean.png"),
            #                nrow=args.nrow)
            #     save_image(watermarked_images[:args.nrow * args.nrow],
            #                os.path.join(args.output_dir, "test_samples_watermarked.png"),
            #                nrow=args.nrow)
